...
inline void SiftGPU::InitSiftGPU()
{
	...
	ParseSiftParam();
std::cout<<"initsiftgpu---1"<<std::endl;
#if !defined(CUDA_SIFTGPU_ENABLED)
...
std::cout<<"initsiftgpu---2"<<std::endl;
	GlobalUtil::InitGLParam(0);
	}
	if(GlobalUtil::_GoodOpenGL == 0)
	{
std::cout<<"initsiftgpu---3"<<std::endl;
	...
	{
std::cout<<"initsiftgpu---4"<<std::endl;
		std::cerr << "Switch from CUDA to OpenGL\n"; 
		GlobalUtil::_UseCUDA = 0;
	}
#endif

//std::cout<<"initsiftgpu---5"<<std::endl;
...
//std::cout<<"initsiftgpu---6"<<std::endl;
	else 
#endif
#if defined(CL_SIFTGPU_ENABLED)
		if(GlobalUtil::_UseOpenCL)
			_pyramid = new PyramidCL(*this);
std::cout<<"initsiftgpu---7"<<std::endl;
		else 
#endif
...
std::cout<<"initsiftgpu---7"<<std::endl;
	...
	{
std::cout<<"initsiftgpu---8"<<std::endl;
...
int  SiftGPU::RunSIFT( IplImage * img1 )

{
	...
	//	IplImage* img1 = cvCreateImage(cvGetSize(image), IPL_DEPTH_8U, 1);//创建目标图像  
...
//	imgdata= new float[height*width];
//	float *head=imgdata;
//std::cout<<"test123 "<<width<<" "<<height<<std::endl;
	_texImage->_converted_data=new float[height*width];
//	_texImage->_converted_data=imgdata;
//	_texImage->_pixel_data=imgdata;//_texImage->_converted_data;

	//将图像的各个像素的数据提取出来保存到imgdata中
	///*   
	int step=0;
std::cout<<step<<std::endl;
	for(y = 0;y<img1->height;y++){
		char *ptr= img1->imageData + y * img1->widthStep;
		for( x = 0;x<img1->width;x++){
			//int temp=ptr[3*x];
			//ptr[3*x]=ptr[3*x+1]=ptr[3*x+2]=temp; //这样就可以添加自己的操作，这里我使三通道颜色一样，就彩色图转黑白图了
			_texImage->_converted_data[step]=((unsigned char)ptr[x])/255.0;	
			step++;	
			//*imgdata=ptr[x]/255.0;		
			//imgdata[x*y] = ptr[3*x+1]/255.0;
//			if(x==168&&y==0)
//				std::cout<<"haha:  "<< imgdata[168] <<" "<<ptr[x]/255.0<<" "<<(int)ptr[x];
		}
	}//*/

	_texImage->_pixel_data=_texImage->_converted_data;//imgdata;//_texImage->_converted_data;
//	std::cout<<" haha:  "<< imgdata[168] <<" ";
...
//	if(GlobalUtil::_GoodOpenGL ==0 ) return 0;i
//InitSiftGPU();
//	if(!_initialized) {InitSiftGPU();std::cout<<"initsiftgpu"<<std::endl;}
//	else GlobalUtil::SetGLParam();
//	if(GlobalUtil::_GoodOpenGL ==0 ) return 0;
//mark123
std::cout<<"test123 "<<width<<" "<<height<<" "<<step<<"init siftgpu"<<std::endl;


	_texImage->_down_sampled=0;
	_texImage->_rgb_converted=2;
	_texImage->_data_modified=1;
	_texImage->_imgWidth=_texImage->_texWidth=_texImage->_drawWidth=width;
	_texImage->_imgHeight=_texImage->_texHeight=_texImage->_drawHeight=height;

std::cout<<"test123456 "<<width<<" "<<height<<" "<<step<<std::endl;
std::cout<<"RunSIFT test -----1"<<std::endl;
std::cout<<"RunSIFT test -----2"<<std::endl;
///*	if(GlobalUtil::_GoodOpenGL ==0 ) return 0;
//	if(!_initialized) InitSiftGPU();
//	else GlobalUtil::SetGLParam();
//	if(GlobalUtil::_GoodOpenGL ==0 ) return 0;
...
//
	//if the size of image is different, the pyramid need to be reallocated.
//	GlobalUtil::StartTimer("Initialize Pyramid");
std::cout<<"before initpyramid "<<std::endl;
if(!_initialized) InitSiftGPU();
	_pyramid->InitPyramid(width, height, _texImage->_down_sampled);
//	GlobalUtil::StopTimer();
//	_timing[1] = GlobalUtil::GetElapsedTime();
...
int SiftGPU::RunSIFT()
{
	...
	//std::cout<<"before----init siftgpu() "<<_initialized<<std::endl;

//	std::cout<<"before----init siftgpu() _image_loaded "<<_image_loaded<<std::endl;
	ClockTimer timer;
	if(!_initialized)
	{
...
//		std::cout<<"if imageload==0 "<<std::endl;
	}else
	{

//		std::cout<<"if imageload ==0 after else "<<std::endl;
...
//	std::cout<<"ifdefine debug_siftgpu "<<std::endl;
...
/*
	fstream f1;
	f1.open("/home/wwd/image1.txt",ios::out);
	if(!f1){
		std::cout<<"can't open the file"<<std::endl;

	}

	int lengthsz=_texImage->_imgHeight*_texImage->_imgWidth;
	std::cout<<"the image's width *height: "<<lengthsz<<std::endl;
	for(int i=0;i<lengthsz;i++){
		f1<<_texImage->_converted_data[i] <<" ";
	}
...
*/
	...
	//<<_texImage->_converted_data[0]<<std::endl;
	printf("%f\n",_texImage->_converted_data[0]);
